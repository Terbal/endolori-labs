...
app.post('/api/auth/login', (req, res) => {
  ...
  if (result.ok) {
    ...
    res.cookie(auth.COOKIE_NAME, token, {
      httpOnly: true,
      secure: true,
      // 'none' is required because the login request itself comes from a
      // different origin (the separate frontend site). The cookie is only
      // ever actually *used* same-origin afterwards (direct navigation to
      // this backend's own /fortuna_major page), so this stays safe.
      sameSite: 'none',
  ...
  if (result.reason === 'not_configured') {
    return res.status(500).json({ error: 'Admin credentials are not configured on the server (ADMIN_USERNAME / ADMIN_PASSWORD).' });
  }
...
const LEAD_STATUSES = ['nouveau', 'a_contacter', 'contacte', 'en_discussion', 'proposition_envoyee', 'gagne', 'perdu'];
...
app.patch('/api/leads/:id', auth.requireAuth, async (req, res) => {
  ...
  if (!Object.keys(updates).length) {
    return res.status(400).json({ error: 'Nothing to update — send "status" and/or "archived".' });
  }
...
app.get('/', (req, res) => {
  res.type('text/plain').send('Endolori Labs API is running. The public site is hosted separately.');
});
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`Endolori Labs backend running on port ${PORT} (storage: ${db.useFirestore ? 'Firestore' : 'local JSON'})`);
});
