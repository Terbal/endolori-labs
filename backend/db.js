...
if (useFirestore) {
  ...
  db = admin.firestore();
  console.log('[db] Firebase env vars detected — using Firestore for persistent storage.');
} else {
  console.log('[db] No Firebase env vars set — using local JSON files under data/. See README.md to switch to Firestore.');
  if (!fs.existsSync(DATA_DIR)) fs.mkdirSync(DATA_DIR, { recursive: true });
  if (!fs.existsSync(STATS_FILE)) fs.writeFileSync(STATS_FILE, JSON.stringify(defaultStats(), null, 2));
...
function jsonReadStats() {
  try {
    return { ...defaultStats(), ...JSON.parse(fs.readFileSync(STATS_FILE, 'utf-8')) };
  } catch (e) {
    return defaultStats();
  }
}
function jsonWriteStats(stats) {
  statsWriteQueue = statsWriteQueue.then(() => fs.promises.writeFile(STATS_FILE, JSON.stringify(stats, null, 2)));
...
function jsonWriteLeads(leads) {
  leadsWriteQueue = leadsWriteQueue.then(() => fs.promises.writeFile(LEADS_FILE, JSON.stringify(leads, null, 2)));
...
async function bumpStats(dotPath, amount) {
  if (useFirestore) {
    ...
    return db.runTransaction(async (tx) => {
      ...
      const stats = doc.exists ? { ...defaultStats(), ...doc.data() } : defaultStats();
...
module.exports = { getStats, bumpStats, resetStats, getLeads, addLead, updateLead, deleteLead, useFirestore };
